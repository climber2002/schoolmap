class AddRandomPointFunction < ActiveRecord::Migration
  def up
  ...
  def down
    connection = ActiveRecord::Base.connection()
    connection.execute <<-SQL
      drop function if exists RandomPointMulti(Geometry);
      drop function if exists RandomPoint(Geometry, INTEGER);
    SQL
  end
end
